package com.ice.seed.common.persistence.pager;
...
public class PageList<T> implements Serializable {
    ...
    public List<Integer> getPageArray() {
        return buildPageArray2(totalPage, pageIndex);
    }
    ...
    /**
     * 构造分页数组
     * 形成如同 34567 的数组，用于形成 34567式的分页视图
     *
     * @param total
     * @param current
     * @return
     */
    private List<Integer> buildPageArray2(Integer total, Integer current) {
        ArrayList<Integer> result = new ArrayList<Integer>();
        if (total < 7) {
            for (int i = 1; i <= total; i++)
                result.add(i);
        }else if(current-3>0&&current+3<=total){
